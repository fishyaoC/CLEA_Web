    ...
    public class AssignClassService : BaseService
    {
        ...
        public IPagedList<AssignClassViewModel.CL> GetClPageList(Guid E_ID, AssignClassViewModel.CLschItem data, Int32 page)
        {
            ...
            result = (from CL in db.ViewBAssignClassLectors
                      ...
                      (
                      (CL.EId == E_ID) &&
                      (string.IsNullOrEmpty(data.Status) || CL.Status.ToString().Equals(data.Status)) &&
        ...
        {
            List<SelectListItem> result = new List<SelectListItem>();
            result.Add(new SelectListItem() { Text = "全部", Value = string.Empty });
            for (int i = 0; i <= 4; i++)
            {
                String[] TextName = new string[] { "待檔案上傳", "待指定評鑑教師", "待評分", "待承辦人審查", "結案" };
                result.Add(new SelectListItem() { Text = TextName[i], Value = i.ToString() });
            }
            return result;
        }
        #endregion
