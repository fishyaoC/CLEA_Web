//------------------------------------------------------------------------------
// <auto-generated>
//     這個程式碼是由範本產生。
//
//     對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//     如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace Clea_Trans
{
    using System;
    using System.Collections.Generic;
    
    public partial class SYS_File
    {
        public System.Guid File_ID { get; set; }
        public string F_Module { get; set; }
        public System.Guid F_MatchKey { get; set; }
        public string F_MatchKey2 { get; set; }
        public string F_FullName { get; set; }
        public string F_NameReal { get; set; }
        public string F_NameDL { get; set; }
        public string F_MimeType { get; set; }
        public string F_Ext { get; set; }
        public string F_Path { get; set; }
        public string F_Description { get; set; }
        public Nullable<int> F_Order { get; set; }
        public string F_Remark { get; set; }
        public System.Guid CREUSER { get; set; }
        public System.DateTime CREDATE { get; set; }
        public Nullable<System.Guid> UPDUSER { get; set; }
        public Nullable<System.DateTime> UPDDATE { get; set; }
    }
}
