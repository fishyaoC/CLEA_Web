...
namespace Clea_Web.Controllers
{
	...
	public class B_AssignClassController : BaseController
	{
		...
		public IActionResult Export_ScoreWord(Guid ED_ID)
		{
			...
			SysFile? sysFile = db.SysFiles.Where(x => x.FMatchKey == viewBAssignClassScore.Evaluate).FirstOrDefault() ?? null;
			...
			using (DocX doc = DocX.Load(SourcePath))
			{
				...
				if (!string.IsNullOrEmpty(PicWrite))
				{
					using (MemoryStream ms = new MemoryStream())
					{
						...
						var paragraphs = doc.Paragraphs.Where(x => x.Text.Contains("[@Evaluater$]"));
			...
			else
			{
				...
				if (eEvaluationSche != null)
				{
					...
					else
					{
						...
						eEvaluationSche.Status = vmd.IsClose ? 4 : 3;
		...
		public IActionResult ScoreExport(Guid ES_ID)
		{
			...
			using (DocX doc = DocX.Load(SourcePath))
			{
				...
				if (sysFile2 != null)
				{
					...
					if (!string.IsNullOrEmpty(PicEvName))
					{
						using (MemoryStream ms2 = new MemoryStream())
						{
							System.Drawing.Image myImg = System.Drawing.Image.FromFile(PicEvName);
							myImg.Save(ms2, myImg.RawFormat);
							ms2.Seek(0, SeekOrigin.Begin);
							var docxImage2 = doc.AddImage(ms2);
